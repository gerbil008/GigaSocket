...
class GigaSocketTLS {
  ...
  public:
    ...
    GigaSocketTLS(int port, int max_conns, std::string key, std::string cert, std::function<void(SSL*, int, std::string)> messagecallback, std::function<void(int)>  closecallback) : onmessage(messagecallback),  onclose(closecallback), port(port), max_conns(max_conns) {
        SSL_library_init();
    ...
    std::string read_msg(SSL* ssl, int cumfd) {
        ...
        if (err <= 0) {
            ...
            log("SSL_read failed with error: " + std::to_string(ssl_err));
            ERR_print_errors_fp(stderr);
        }
...
class GigaSocket {
  ...
  public:
    ...
    GigaSocket(int port, int max_conns, std::function<void(int, std::string)> messagecallback, std::function<void(int)>  closecallback) : onmessage(messagecallback),  onclose(closecallback), port(port), max_conns(max_conns) {
        sockfd = socket(AF_INET, SOCK_STREAM, 0);
        bzero(&servaddr, sizeof(servaddr));
        servaddr.sin_family = AF_INET;
        servaddr.sin_addr.s_addr = htonl(INADDR_ANY);
